...
class PaperExecutionProvider(ExecutionProvider):
    """Paper-only execution — simulates spot trades with fees and slippage."""
    ...
    def _buy(
        ...
        if state.in_position:
            return state, ExecutionResult(False, "Bereits in Position")

        trade_budget = min(self.max_trade_eur, state.cash_eur)
        if trade_budget <= 0:
            return state, ExecutionResult(False, "Kein verfügbares EUR-Guthaben")

        execution_price = self.apply_slippage(signal.price, "buy")
        fee_eur = trade_budget * (self.fee_pct / 100)
        net_eur = trade_budget - fee_eur
        if net_eur <= 0:
            return state, ExecutionResult(False, "Trade zu klein nach Gebühren")
        ...
        return updated, ExecutionResult(True, signal.reason)
    ...
    def _sell(
        ...
        if not state.in_position or state.btc_amount <= 0:
            return state, ExecutionResult(False, "Keine Position zum Verkaufen")
